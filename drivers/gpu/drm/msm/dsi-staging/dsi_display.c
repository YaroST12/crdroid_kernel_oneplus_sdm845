...
int dsi_display_set_customer_p3_mode(struct drm_connector *connector, int level)
{
	...
	if ((connector == NULL) || (connector->encoder == NULL) || (connector->encoder->bridge == NULL))
		return 0;
	...
	rc = dsi_display_clk_ctrl(dsi_display->dsi_clk_handle, DSI_CORE_CLK, DSI_CLK_ON);
	...
	rc = dsi_display_clk_ctrl(dsi_display->dsi_clk_handle, DSI_CORE_CLK, DSI_CLK_OFF);
